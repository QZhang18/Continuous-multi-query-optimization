...
inline void readDataGraph(string datagraphFile) {
	...
	while (getline(fin,line))
	{
		...
		if (line.at(0) == 'e')
		{
			...
			if (left_node != right_node) {//put the nodes into the adjacent list，根据表构建邻接表
				...
				//while (map_it != edge_label.end())//遍历整个索引表输出，因为MAP的键值是严格弱排序，因此输出是字典序  
				//{
...
inline void constructAQG(string querygraphFile) {
	...
	while (getline(fin, line))
	{
		if (line.at(0) == 't') {
			...
			label_id_c = new int[cnt_node_query_c];//相同的标签用不同的ID标记
		}
		if (line.at(0) == 'v') {
			split(line, v, ' ');
			int label = atoi(v[2].c_str());
			...
			label_id_c[node_id] = duplicate_label;//计算新的查询中每个节点的标签信息
		}
		...
		if (line.at(0) == 'e')
		{
			...
			if (left_node != right_node) {//put the nodes into the adjacent list，根据表构建邻接表
				...
				{
					if (nodes_label_query[i]== nodes_label_query_c[left_node]&&label_id[i]==label_id_c[left_node])//左边节点
					{
						aa++;
					...
					if (nodes_label_query[i] == nodes_label_query_c[right_node] && label_id[i] == label_id_c[right_node])//右边节点
					{
						aa++;
						for (int j = 0; j < query_nodes_c[i].size(); j++)
						{
...
inline void labelAQG(string querygraphFile) {
	...
	for (int i = 0; i < cnt_node_query; i++)
	{
		query_edge_id[i].resize(query_nodes[i].size());//为AQG的每一个边辅助查询id
	}
	query_nodes.clear();
	query_nodes.resize(cnt_node_query);
	cnt_node_query = 0;
...
inline void coreDecomposition_query() {
	...
	for (int i = 0; i < cnt_node_query; i++) {
		...
		for (int j = query_nodes_array_info[v]; j < query_nodes_array_info[v + 1]; j++) {
			...
			int u = query_nodes_array[j]; // nodes_query[v][j];v的邻接表中的顶点

			if (core_number_query[u] > core_number_query[v]) {

				int du = core_number_query[u];//顶点的邻接点的度比顶点大的顶点的度
				int pu = pos[u];

				int pw = bin[du];//<pos[u]>找到跟u的度相同的顶点
				int w = vert[pw];//<u>

				if (u != w) {	//if not the same node, switch the position of the two nodes.？为啥要转换位置
					pos[u] = pw;
					pos[w] = pu;
					vert[pu] = w;
					vert[pw] = u;
	...
	*/
	...
	while (true) {
		...
		while (pointer_this_start != pointer_this_end) { // queue not empty
			...
			int end = query_nodes_array_info[current_node + 1];//构建BFS树
			...
			for (int i = start; i < end; i++) {
				...
				if (visited[childNode] != 0) { //this child node has been visited before,
					if (childNode != core_query_tree[current_node].parent_node)//在同一级上
						addNonTreeEdgeToTreeNode(core_query_tree[current_node], childNode);//找到Nte边！！！！！S-NTE
					...
					visited[childNode] = current_level + 1; //parent node's level plus one当前等级的是父节点
		...
		level_index.push_back(make_pair(start, end));//往容器里添加元素

		for (int i = end - 1; i >= start; i--) {
			int node = simulation_sequence[i];
			if (core_number_query[node] < 2)
				continue;
			exploreCRSequence[exploreCRSequence_indx++] = node;//主要是用来对N-TREE边的核实
		}
...
//					if (str3 == str4&&std::find(is_path[child].begin(), is_path[child].end(), candidate_c) == is_path[child].end()) //需要判断这个元素是否存在
//					{
...
inline void constructMDCG() {
	...
	/********************************构建初始辅助数据结构，以查询为中心***********************************************************/
	CAND.resize(cnt_node_query);//获得每一个查询节点的候选集
	for (int  i = 0; i <cnt_node_query; i++)
	{
		for (int j = 0; j < cnt_node; j++)
		{
	...
	for (int i = 0; i <cnt_node_query; i++)
	{
		...
		for (int j = 0; j < CAND[i].size(); j++)
		{
			...
			for (int n = 0; n <query_num; n++)
			{
				...
				stree_or_match[i][j][n].push_back(0);//为每一个节点构建一个状态标识符来说明其当前的状态. 对应的查询是满足子树匹配还是满足树匹配
			}
		}
		...
		else
		{
			...
			for (int j = 0; j < BFS_child[cur].size(); j++)
			{
				...
				for (int k = 0; k < query_nodes[cur].size(); k++)
				{
					if (query_nodes[cur][k]==child)
					{
						...
						{
							visited1[query_edge_id[cur][k][m]]++;//判断孩子节点中存在的查询
						}
					}
				}
			}
			for (int j = 0; j < query_num; j++)
			{
				...
				if (visited1[j]>0)
				{
					...
					{
						if (verify_query(cur, BFS_child[cur][k],j))//用于核实两个边之间是否存在对应的查询id
						{
							temp1++;
		...
		for (int j = 0; j < BFS_parent[cur].size(); j++)
		{
			...
			for (int k = 0; k < query_nodes[cur].size(); k++)
			{
				if (query_nodes[cur][k] == parent)
				{
					for (int m = 0; m < query_edge_id[cur][k].size(); m++)
					{
						visited1[query_edge_id[cur][k][m]]++;//判断父亲节点中存在的查询，与ORDER2对称
					}
				}
			}
			for (int j = 0; j < query_num; j++)
			{
				...
				if (visited1[j]>0)
				{
					...
					{
						if (verify_query(cur, BFS_parent[cur][k], j))//用于合适两个边之间是否存在对应的查询id
						{
							temp1++;
...
/********************************查找独立节点，将其对应的match设为1**********************************************************/
	...
	for (int i = 0; i < cnt_node_query; i++)
	{
		...
		for (int j = 0; j < query_num; j++)
		{
			if (temp[j]>0&& visited1[j]==0)//查询出现在与孩子节点连接的边中，没有出现在与父亲节点连接的边中
			{
...
inline void updateDOWN(int data_parent, int data_child, int query_parent, int query_child, int id) {
	...
	if (s2 == s3&& stree_or_match[query_child][mm][id][0] == 1)
	{
		...
		for (int i = 0; i < BFS_child[query_child].size(); i++)
		{
			...
			if (verify_query(child, query_child, id)) {
...
inline void GetMatch(int data_parent, int data_child, int query_parent, int query_child, int id) {
	...
	//					{
	//						it = CAND_order1[cur].erase(it); //不能写成arr.erase(it);
	//						break;
	//					}
	//					else
	//					{
	...
	for (int i = 0; i < cnt_node_query; i++)
	{
		if (CAND_order[i].size()>0)//与查询相关的节点
		{
...
/*****************************************计算匹配结果*******************************************************************/
	...
	vector<vector<int> > matches;//存储查询匹配结果
	vector<vector<int> > matches_candidate;//存储查询匹配结果
...
inline void updateUP(int data_parent, int data_child, int query_parent, int query_child, int id) {
	...
	if (s2== s3)
	{
		...
		if (query_parent_verify(query_parent,id))//判断是否为开始节点
		{
			int s = 0; int sss = 0;
			for (int i = 0; i < BFS_parent[query_parent].size(); i++)
			{
...
inline void update_MDCG(int data_parent, int data_child, int query_parent, int query_child) {
	...
	{
		if (verify_query(query_child,query_parent,i))
		{
...
inline void update_processing(string insertionFile) {
	...
	while (getline(fin, line))
	{
		...
		if (line.at(0) == 'e')
		{
			...
			if (left_insertion_node != right_insertion_node) {//put the nodes into the adjacent list，根据表构建邻接表
